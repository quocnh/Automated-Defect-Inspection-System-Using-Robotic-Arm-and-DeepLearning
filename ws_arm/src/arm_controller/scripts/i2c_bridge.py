...
from pca9685_msgs.msg import ServoState
...
def writeData(noun, verb):
    ...
    try:
        ...
        #print("bus.write_i2c_block_data(%s, 0, list(%s))" % (address, bytesToSend))
        using_bus = False
    except OSError:
        global fail_counter
        fail_counter += 1
...
def publishSensors():
    ...
    while not rospy.is_shutdown():
        ...
        # print("jointState", jointState.name)
...
def publishServo(port_num, rot):
    ...
    # rate.sleep()
    servoState.port_num = port_num
    servoState.servo_rot = rot
    servoState.servo_type = 2
    pubServo.publish(servoState)
...
def sendJointCallback(data):
    ...
    for i in range(dataLength):
        ...
        print("data_name", data.name[i])
        print("data_position", data.position[i])
        if data.name[i] == "joint_1":
            deg = int(radian * 180.0 / 3.14 * 100.0)
            print("data_deg", deg)
            publishServo(1, radian)
...
if __name__ == '__main__':
    try:
        rospy.init_node('i2c_bridge')
        rospy.Subscriber("/command/joint_states", JointState,
                         sendJointCallback)
        # rospy.Subscriber("/joint_states", JointState,
        #                  sendJointCallback)
        pubServo = rospy.Publisher('/pca9685/servostate_to_controller', ServoState, queue_size=10)
